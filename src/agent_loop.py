...
Streaming agent loop for TaiAi-ui.
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- TaiAi chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
```
GENERIC LOOPBACK to allowed TaiAi internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if has(r"\b(cookbook|serve|serving|served|launch|start|preset|vllm|sglang|llama\.?cpp|ollama|download|downloading|pull|cached models?|running models?|model servers?|models? (?:are )?running|what models?|model picker|gpu box|kierkegaard|TaiAi|ajax|qwen|gemma|llama|mistral|minimax)\b"):
...
async def stream_agent_loop(
    ...
    slim_profile: Optional[str] = None,  # "auto" | "minimal" | "balanced" | "full" | None
    slim_max_context: Optional[int] = None,  # explicit cap; overrides profile default
    ...
    # Phase 1.3: Slim Agent Mode. When the caller specifies a profile,
    # resolve it against the model + endpoint and apply the profile's tool
    # whitelist (intersected with the already-disabled set) + context cap.
    # None / "auto" / unset -> no change. Profiles are documented in
    # core/agent_profiles.py.
    _slim_profile_resolved = None
    if slim_profile is not None and slim_profile != "auto":
        try:
            from core.agent_profiles import resolve_profile
            _slim_profile_resolved = resolve_profile(
                model_id=model, base_url=endpoint_url or "",
                explicit=slim_profile,
            )
        except Exception as _e:  # noqa: BLE001
            logger.warning("[slim] profile resolution failed for %r: %r", slim_profile, _e)
    elif slim_profile == "auto":
        try:
            from core.agent_profiles import resolve_profile
            _slim_profile_resolved = resolve_profile(
                model_id=model, base_url=endpoint_url or "",
            ...
            logger.debug("[slim] auto-resolve skipped: %r", _e)
    if _slim_profile_resolved is not None:
        prof = _slim_profile_resolved
        # Apply tool whitelist by INVERTING it into disabled_tools.
        # Profile.tool_whitelist=None means "all tools" -> no change.
        if prof.tool_whitelist is not None:
            try:
                from src.tool_security import BUILTIN_TOOL_NAMES
                allowed = set(prof.tool_whitelist)
                # Everything else (built-in + MCP) gets disabled.
                # MCP tools are namespaced "mcp__server__tool"; the whitelist
                # matches against the short name (the part after the last
                # double-underscore).
                from src.mcp_manager import get_mcp_manager as _get_mcp
                _mcp_short = {t.rsplit("__", 1)[-1]: t for t in (
                    _get_mcp().list_tool_names() if _get_mcp() else []
                )}
                blocked_by_slim = set()
                # Built-in tools: disable anything not in allowed.
                for n in BUILTIN_TOOL_NAMES:
                    if n not in allowed:
                        blocked_by_slim.add(n)
                # MCP tools: disable short names not in allowed.
                for short, full in _mcp_short.items():
                    if short not in allowed:
                        blocked_by_slim.add(full)
                disabled_tools.update(blocked_by_slim)
                logger.info(
                    "[slim] profile=%s disabled %d extra tools for model=%s",
                    prof.id, len(blocked_by_slim), model,
                )
            except Exception as _e:  # noqa: BLE001
                logger.debug("[slim] tool whitelist apply skipped: %r", _e)
        # Apply context cap. slim_max_context (caller override) wins.
        cap = slim_max_context if slim_max_context else prof.max_context_tokens
        if cap and context_length > cap:
            logger.info(
                "[slim] profile=%s context_length=%d > cap=%d; will compact",
                prof.id, context_length, cap,
            )
            context_length = cap
        # Disable intent verifier on minimal profiles (per profile flag).
        if not prof.enable_intent_verifier:
            try:
                # Set a flag the verifier checks before running.
                import builtins as _b
                _b.__slim_disable_verifier__ = True  # noqa: SLF001
            except Exception:
                pass
