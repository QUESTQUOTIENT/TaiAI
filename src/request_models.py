...
class ChatRequest(BaseModel):
    ...
    # slim_profile: "auto" (default, resolved from model + hardware) |
    #              "minimal" (small local models) | "balanced" (default for
    #              medium hardware) | "full" (large cloud models). None
    #              means "do not change tool set, leave default behavior".
    slim_profile: Optional[str] = Field(
        default=None,
        description="Slim Agent Mode profile: auto|minimal|balanced|full",
        pattern="^(auto|minimal|balanced|full)$",
    )
    slim_max_context: Optional[int] = Field(
        default=None, ge=1000, le=200000,
        description="Override the profile's default context-token cap.",
    )
