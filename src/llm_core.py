...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt TaiAi' canonical OpenAI-style messages to native Ollama /api/chat.

    TaiAi carries assistant tool calls in the OpenAI shape, where
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the TaiAi session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/TieAI-archdaemon/TaiAi")
        h.setdefault("X-OpenRouter-Title", "TaiAi")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip TaiAi-only metadata before sending messages to providers.
