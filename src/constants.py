...
DATA_DIR = os.getenv("TaiAi_DATA_DIR", os.path.join(BASE_DIR, "data"))

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place TaiAi_DATA_DIR is read. Import these constants instead of
...
MAIL_ATTACHMENTS_DIR = os.getenv("TaiAi_MAIL_ATTACHMENTS_DIR", os.path.join(DATA_DIR, "mail-attachments"))
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to TaiAi's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. TaiAi_INTERNAL_BASE  - explicit override (e.g. behind a TLS proxy).
    ...
    """
    override = os.environ.get("TaiAi_INTERNAL_BASE")
    if override:
