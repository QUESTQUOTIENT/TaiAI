...
def test_admin_only_actions_set_contains_shell_runners():
    ...
    # wording.
    src = Path(task_routes.__file__).read_text(encoding="utf-8")
...
def test_ship_paused_housekeeping_stays_paused_by_default():
    ...
    route_src = Path(task_routes.__file__).read_text(encoding="utf-8")
    scheduler_src = Path(task_scheduler.__file__).read_text(encoding="utf-8")
