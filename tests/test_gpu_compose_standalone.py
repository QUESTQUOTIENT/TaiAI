...
base compose with only the matching overlay merged into the ``TaiAi``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on TaiAi only."""
...
# --- Non-TaiAi services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_TaiAi_services_match_base(base, standalone_path):
...
# --- TaiAi = base service + only the overlay additions ------------------


def test_nvidia_TaiAi_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_TaiAi_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
