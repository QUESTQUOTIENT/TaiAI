...
INTERNAL_TOOL_TOKEN = os.environ.get("TaiAi_INTERNAL_TOKEN") or secrets.token_hex(32)
INTERNAL_TOOL_HEADER = "X-TaiAi-Internal-Token"
...
def require_admin(request: Request):
    ...
    # (a) header-direct (caller set X-TaiAi-Internal-Token), or
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        # Report-Only header so a misbehaving inline event handler surfaces in
        # /api/csp-report without breaking the page. Operators can then harden
        # the enforcing CSP once violations are clean.
        response.headers["Content-Security-Policy-Report-Only"] = (
            "default-src 'self'; "
            "script-src 'self' 'unsafe-inline'; "
            "style-src 'self' 'unsafe-inline'; "
            "report-uri /api/csp-report; report-to /api/csp-report"
        )
