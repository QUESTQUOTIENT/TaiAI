...
def _default_document_owner() -> str | None:
    ...
    """
    owner = os.environ.get("TaiAi_DOCUMENT_OWNER", "").strip()
    if owner:
        return owner
    try:
...
def _create_email_draft_document(
    *,
    ...
    """Create an TaiAi email compose document for user review. Does not send."""
...
def _draft_reply_to_email(uid, body, folder="INBOX", reply_all=False, account=None, title=None):
    """Create a threaded TaiAi reply draft document. Does not send."""
    conn = _imap_connect(account)
    conn.select(_q(folder), readonly=True)
    status, msg_data = conn.uid("FETCH", _b(uid), "(BODY.PEEK[])")
    conn.logout()
...
async def _ai_draft_reply_to_email(uid, folder="INBOX", reply_all=False, account=None, title=None):
    """Generate a reply with TaiAi' AI-reply prompt/style, then create a compose doc."""
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            ...
            description=(
                "List the email accounts configured in TaiAi. Returns each account's "
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email so the user can review and send from TaiAi. "
        ...
        Tool(
            name="draft_email",
            description=(
                "Create a new TaiAi email compose draft document. This DOES NOT send. "
                ...
                "can edit or press Send in TaiAi. "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "cc": {"type": "string", "description": "CC address(es), comma-separated (optional)"},
                    "bcc": {"type": "string", "description": "BCC address(es), comma-separated (optional)"},
                    "title": {"type": "string", "description": "Optional TaiAi document title"},
                    **ACCOUNT_PROP,
                },
                "required": ["to", "subject", "body"],
            },
        ...
        Tool(
            ...
            description=(
                ...
                "review and send from TaiAi. Automatically threads the reply with "
        ...
        Tool(
            ...
            description=(
                "Create an TaiAi email reply draft document for an existing email UID. "
        ...
        Tool(
            ...
            description=(
                "Generate an AI reply using TaiAi' existing AI Reply behavior, "
    ...
    try:
        ...
        if name == "list_emails":
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created TaiAi email draft `{result['title']}` "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in TaiAi to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created TaiAi reply draft `{result['title']}` for UID {uid} "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in TaiAi to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Generated AI reply and created TaiAi compose draft "
