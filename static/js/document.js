...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__TaiAiActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'TaiAi-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'TaiAi-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__TaiAiActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('TaiAi-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('TaiAi-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'TaiAi-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('TaiAi-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('TaiAi-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('TaiAi-suggestions-' + docId);
      if (!raw) return;
