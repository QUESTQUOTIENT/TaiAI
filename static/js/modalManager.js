...
const _rememberedDockKey = (id) => `TaiAi-modal-remembered-dock-${id}`;
...
function _emitModalOpened(id, modal) {
  try {
    window.dispatchEvent(new CustomEvent('TaiAi:modal-opened', {
      detail: { id, modal },
    }));
  } catch (_) {}
}
...
const _LABELS = {
  ...
  // Full SVG markup (multi-path) so the dock renderer keeps the info-circle glyph.
  'info-modal':        { label: 'Info',      icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>' },
...
const _DOCK_STORAGE_KEY = 'TaiAi.mobileDockState.v1';
...
const _AUTO_WIRE = {
  ...
  'info-modal':           { rail: 'rail-info',      sidebar: 'tool-info-btn' },
