...
export const THEMES = {
  ...
  // fg was #e94560 (~4.46:1 against bg, fails WCAG AA). Bumped to
  // #ff5570 (~5.5:1) — passes AA + large-text bar.
  retrowave:  { bg:'#1a1a2e', fg:'#ff5570', panel:'#16213e', border:'#533483', red:'#e94560' },
  ...
  // fg was #d4608a (~3.5:1 against bg, fails WCAG AA). Bumped to #a93d6b
  // (~6:1 against bg, passes AA + AAA for large text). Visual feel retained
  // because the accent (#ff6b9d) carries the pink.
  cute:       { bg:'#fff0f5', fg:'#a93d6b', panel:'#fff8fa', border:'#e8b5c7', red:'#ff6b9d' },
};

// First-time default for users who have never picked a theme. Cyberpunk
// (neon purple + cyan on near-black) — the other 16 built-ins remain
// available from the theme picker for users who want a different vibe.
const DEFAULT_THEME = 'cyberpunk';
const LS_KEY = 'TaiAi-theme';
const CUSTOM_THEMES_KEY = 'TaiAi-custom-themes';
...
// Short descriptor shown under each preset swatch. Borrowed from the
// "Appearance & Themes" card of the Cronos Studio app — turns the grid
// into a readable catalogue rather than a wall of identically-named chips.
// Keys that don't appear here fall back to a sensible default in
// _describePreset().
const THEME_DESCRIPTIONS = {
  dark:       'Original dark theme',
  light:      'Light mode, warm tones',
  midnight:   'Deep black + cyan',
  paper:      'Soft warm whites',
  cyberpunk:  'Neon purple + cyan',
  retrowave:  'Magenta + violet',
  forest:     'Navy + green',
  ocean:      'Deep blue gradient',
  terminal:   'Pure black + green',
  organs:     'Dark + crimson',
  ume:        'Plum + pink',
  copper:     'Earth + amber',
  lavender:   'Lilac + cream',
  gpt:        'ChatGPT greyscale',
  claude:     'Claude warm dark',
  cute:       'Soft pink, light',
};
...
const ADV_KEYS = [
  ...
  { key: 'brandColor',         css: '--brand-color',       label: 'TaiAi Logo',    group: 'Sidebar' },
...
function syncPickers(colors) {
  ...
  // Mirror the picker values into the hex text inputs so the two stay
  // in lockstep (the user can type into either to update the theme).
  document.querySelectorAll('.theme-hex-input[data-hex-for]').forEach((hexIn) => {
    const p = document.getElementById(hexIn.dataset.hexFor);
    if (p) { hexIn.value = p.value || ''; hexIn.classList.remove('invalid'); }
  });
}
...
export function initThemeUI() {
  ...
  // Render preset swatches — each chip carries a 4-color preview, the
  // preset's display name, a short descriptive caption, and an "active"
  // dot in the top-right that appears when this preset is currently
  // selected. The dot complements the existing red border so the active
  // state is unambiguous even in monochrome screenshots.
  function _describePreset(name) {
    if (THEME_DESCRIPTIONS[name]) return THEME_DESCRIPTIONS[name];
    // Sensible default for any future preset we forget to label.
  ...
  function _displayName(name) {
    ...
    if (name === 'ume')   return 'ume';
    return name;
  }
  grid.innerHTML = Object.entries(THEMES).map(([name, c]) => `
    <div class="theme-swatch${name === activeName ? ' active' : ''}" data-theme="${name}" title="${_describePreset(name)}">
      <span class="theme-swatch-active-dot" aria-hidden="true"></span>
      ...
      <span class="theme-swatch-name">${_displayName(name)}</span>
      <span class="theme-swatch-desc">${_describePreset(name)}</span>
    </div>
  `).join('');
  ...
  if (customEntries.length > 0 && userGrid && userCard) {
    ...
    userGrid.innerHTML = customEntries.map(([name, c]) => `
      <div class="theme-swatch${name === activeName ? ' active' : ''}" data-theme="${name}" data-custom="1" title="Custom theme · ${name}">
        <span class="theme-swatch-active-dot" aria-hidden="true"></span>
        ...
        <span class="theme-swatch-desc">Custom · saved to browser</span>
      </div>
    `).join('');
  } else if (userCard) {
  ...
  // Active-theme status pill. Updates whenever the swatch click handler
  // fires, and once on init so the pill isn't blank when the popup first
  // opens.
  function _renderStatusPill(name, source) {
    const pill  = document.getElementById('themeStatusPill');
    const swatch= document.getElementById('themeStatusSwatch');
    const label = document.getElementById('themeStatusName');
    const meta  = document.getElementById('themeStatusMeta');
    if (!pill || !label || !meta) return;
    const isCustom = source === 'custom';
    const themeColors = isCustom ? customThemes[name] : THEMES[name];
    const isBuiltIn = !!THEMES[name];
    const pretty = isCustom ? name : (isBuiltIn ? _displayName(name) : (name || '—'));
    label.textContent = pretty;
    meta.textContent = isCustom ? 'Custom · saved to browser' : (isBuiltIn ? 'Built-in preset' : 'Active theme');
    if (swatch) {
      if (themeColors) {
        const grad = `linear-gradient(135deg, ${themeColors.bg} 0%, ${themeColors.bg} 50%, ${themeColors.red} 50%, ${themeColors.red} 100%)`;
  ...
  _renderStatusPill(activeName, customThemes[activeName] ? 'custom' : 'builtin');
  ...
  allGrids.forEach(g => {
    g.querySelectorAll('.theme-swatch').forEach(sw => {
      sw.addEventListener('click', (e) => {
        ...
        _renderStatusPill(name, sw.dataset.custom ? 'custom' : 'builtin');
        syncPickers(colors);
  ...
  // ── Hex text inputs alongside each color picker ──
  // Power users can paste a hex code (#abc, #aabbcc, AABBCC) and it
  // applies live, mirroring the corresponding color picker. We sync BOTH
  // directions: typing into the text input updates the picker (which
  // re-triggers the picker input handler so the live-apply runs), and
  // picking via the swatch keeps the text input's value in lockstep.
  function _normalizeHexInput(raw) {
  ...
  function _isValidHex(raw) {
    return /^#[0-9a-f]{6}$/i.test(String(raw || '').trim());
  }
  document.querySelectorAll('.theme-hex-input[data-hex-for]').forEach((hexIn) => {
    if (hexIn.dataset.hexBound === '1') return;
    hexIn.dataset.hexBound = '1';
    const pickerId = hexIn.dataset.hexFor;
    const picker   = document.getElementById(pickerId);
    if (!picker) return;
    // Initialise the text input from the picker.
    hexIn.value = picker.value || '';
    const _apply = () => {
      // Accept either 3-char (#abc) or 6-char (#aabbcc) hex. We always
      // normalize before comparing so case + shorthand both work.
      const norm = _normalizeHexInput(hexIn.value);
      if (!_isValidHex(norm)) {
        hexIn.classList.add('invalid');
        return;
      }
      hexIn.classList.remove('invalid');
      hexIn.value = norm; // reflect the expanded form back to the user
      if (norm === (picker.value || '').toLowerCase()) return;
      picker.value = norm;
      picker.dispatchEvent(new Event('input', { bubbles: true }));
    };
    hexIn.addEventListener('input', () => {
      // Live preview: don't commit invalid input but show the red border.
      hexIn.classList.toggle('invalid', !!hexIn.value && !_isValidHex(hexIn.value));
    });
    hexIn.addEventListener('change', _apply);       // fires on blur/Enter
    hexIn.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') { e.preventDefault(); _apply(); hexIn.blur(); }
    });
    // Mirror picker → text whenever the picker changes.
    if (picker.dataset.hexMirrorBound !== '1') {
      picker.dataset.hexMirrorBound = '1';
      picker.addEventListener('input', () => {
        if (document.activeElement === hexIn) return; // user is typing
        hexIn.value = picker.value || '';
        hexIn.classList.remove('invalid');
      });
    }
  ...
  // ── "Re-apply All" button ──
  // Reads the saved theme from localStorage and re-pushes every color to
  // the CSS variables in one shot. Useful after closing/reopening the
  // popup if a background effect has drifted the palette, or after a
  // page reload that didn't apply the user's saved theme.
  const reapplyBtn = document.getElementById('theme-reapply-btn');
  if (reapplyBtn && !reapplyBtn.dataset.bound) {
    reapplyBtn.dataset.bound = '1';
    reapplyBtn.addEventListener('click', () => {
      const s = getSaved();
      const colors = s ? s.colors : THEMES[DEFAULT_THEME];
      if (!colors) return;
      applyColors(colors);
      syncPickers(colors);
      // Push the same colors into every hex text input too.
      document.querySelectorAll('.theme-hex-input[data-hex-for]').forEach((hexIn) => {
        const p = document.getElementById(hexIn.dataset.hexFor);
        if (p) { hexIn.value = p.value || ''; hexIn.classList.remove('invalid'); }
      });
      if (uiModule && uiModule.showToast) uiModule.showToast('Theme re-applied');
    });
  }
  ...
  if (exportBtnEl) {
    ...
    newExp.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = 'TaiAi_' + (obj.name || 'theme') + '.json';
      a.click();
