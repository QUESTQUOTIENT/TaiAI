...
const RECENT_KEY = 'TaiAi-model-recent';
const FAVORITES_KEY = 'TaiAi-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('TaiAi-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('TaiAi-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('TaiAi:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('TaiAi:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `TaiAi-model-favorites[0]`
