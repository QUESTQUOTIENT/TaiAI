...
export function initSidebarLayout(Storage, opts) {
  ...
  function _syncRailSideCore() {
    ...
    if (!iconRail) return;
    // Two independent ways the rail/sidebar can end up on the right:
    //   - isRight: the per-element "right-side" class on the sidebar
    //     (set by shift-clicking the hamburger to flip the sidebar to
    //     the right while keeping the rail+sidebar next to each other)
    //   - isFlipped: the global "layout-flipped" class on body
    //     (set by Settings → "Flip layout" — mirrors the whole UI)
    // Both apply; combined they decide which side the chrome sits on.
    const isRight = sidebar.classList.contains('right-side');
    const isFlipped = document.body.classList.contains('layout-flipped');
    const effectiveRight = isRight || isFlipped;
    ...
    iconRail.classList.toggle('right-side', effectiveRight);
    ...
    // Hamburger button — visible side tracks the rail's effective side
    // (not just the per-element right-side class) so the padding rules
    // `body.sidebar-collapsed.hamburger-right .chat-top-bar { padding-right: 38px; }`
    // apply correctly in layout-flipped mode.
    if (hamburgerBtn) {
      document.body.classList.toggle('hamburger-right', effectiveRight);
      document.body.classList.toggle('hamburger-left', !effectiveRight);
    ...
    // Keep incognito button clear of hamburger (the hamburger is on the
    // edge opposite the rail in layout-flipped mode, so the incognito
    // nudge needs the matching offset)
    const incogBtn = document.getElementById('incognito-btn');
    if (incogBtn) {
      if (effectiveRight && sidebarHidden) {
