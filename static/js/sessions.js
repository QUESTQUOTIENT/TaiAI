...
let _sortMode = Storage.get('TaiAi-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'TaiAi Chat';
...
const FOLDER_STATE_KEY = 'TaiAi-folder-state';
const FOLDER_ORDER_KEY = 'TaiAi-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to TaiAi kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'TaiAi Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('TaiAi-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('TaiAi-session-sort', mode);
  else Storage.remove('TaiAi-session-sort');
  renderSessionList();
}
