...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('TaiAi:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('#adm-add-api, #adm-add-local').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'TaiAi.addModels.' + sec.id + '.open';
    let open = false;
...
6. Set Google Oauth Credentials to the full path of the downloaded JSON file` },
  ...
  { name: "Browser (Playwright)", command: "npx", args: ["-y", "@playwright/mcp@latest", "--headless", "--caps", "vision"],  env: {},
    help: "Browser automation via Playwright. The AI can navigate pages, click, fill forms, and read content.\nRuns headless by default. Remove --headless from Args to see the browser window.\nFirst run installs Chromium automatically.\n\nTip: the --caps vision flag lets the model SEE rendered pages (essential for modern JS-heavy sites, image CAPTCHAs, and screenshots). Don't remove it unless you know why." },
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'TaiAi_backup.json';
...
function initAll() {
  ...
  const inits = [initSignupToggle, initAddUser, initEndpointForm, initMcpForm, initCalDAV, initBackup, initDangerZone, initTokenForm, initWebhookForm, initDiagnostics, () => settingsModule.initIntegrations()];
