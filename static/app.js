// ============================================
// TaiAi UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__TaiAiDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'TaiAi Chat';
  ...
  // ── Info & Feature Guide modal toggle ──
  const toolInfoBtn = el('tool-info-btn');
  if (toolInfoBtn) {
    toolInfoBtn.addEventListener('click', async () => {
      // Try minimized→restore or open→minimize via the manager first
      const Modals = await import('./js/modalManager.js');
      if (!Modals.toggle('info-modal')) {
        // Not registered yet, or already open — open (idempotent). The
        // module's open() reuses the existing DOM so a second click on an
        // already-visible Info modal is a no-op (avoids the re-render race
        // that the original code hit when Modals.toggle() returned false
        // while the modal was already visible).
        const info = await import('./js/info.js');
        if (info && typeof info.isOpen === 'function' && info.isOpen()) return;
        info.open();
      }
    });
  }
  ...
  const _routeOpen = {
    ...
    '/info':     () => document.getElementById('tool-info-btn')?.click(),
  };
  ...
  if (_opener) window._TaiAiRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('TaiAi-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'TaiAi-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message TaiAi...');
      }
  ...
  const UI_VIS_KEY = 'TaiAi-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'TaiAi-toolbar-visibility';
    try {
...
// ============================================
function startTaiAiApp() {
  if (window.__TaiAiAppStarted) return;
  window.__TaiAiAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('TaiAi-doc-open-' + _curSession) === '1') {
  ...
  window._TaiAiBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._TaiAiRouteOpener) {
          try { window._TaiAiRouteOpener(); } catch (_) {}
          window._TaiAiRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startTaiAiApp, { once: true });
} else {
  startTaiAiApp();
}
