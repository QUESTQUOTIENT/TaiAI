...
def _container_loopback_reachable(base_url: str, timeout: float = 0.2) -> bool:
    ...
    This distinguishes "a model server running alongside TaiAi in the same
...
def _rewrite_loopback_for_docker(base_url: str, *, container_local: bool = False) -> str:
    ...
    default) otherwise targets the TaiAi container itself, so the probe gets
    a connection error and the endpoint is rejected with a misleading "No
    models found for that provider/key".

    Cookbook local serves are the opposite case: TaiAi started the model
        ...
        if 300 <= r.status_code < 400:
            ...
            if loc.startswith("/login") or "/login" in loc:
                return {
                    ...
                    "error": "That is TaiAi, not a model server. Use the Ollama URL, usually http://host.docker.internal:11434/v1 in Docker.",
                }
...
def setup_model_routes(model_discovery):
    ...
    def create_model_endpoint(
        ...
        # server. Cookbook local serves are launched inside TaiAi itself, so
    ...
    # ============================================================
    # Free provider registry — one-click add popular free chat APIs.
    # See src/free_providers.py for the registry contents.
    # ============================================================
    @router.get("/free-providers")
    def list_free_providers(request: Request):
        """Return the registry of free chat-completion providers that
        the admin can add as endpoints with one click. Each entry is
        a card the Settings → Models UI shows as a 'Free providers' tab."""
        from src.free_providers import list_providers, provider_card_data
        require_admin(request)
        out = []
        for p in list_providers():
            card = provider_card_data(p)
            # Already-added?
            db = SessionLocal()
            try:
                base = p["base_url"]
                row = db.query(ModelEndpoint).filter(ModelEndpoint.base_url == base).first()
                card["already_added"] = bool(row)
                card["existing_endpoint_id"] = row.id if row else None
            finally:
                db.close()
            out.append(card)
        return {"providers": out}

    @router.post("/free-providers/{provider_id}/add")
    def add_free_provider(
        provider_id: str,
        request: Request,
        api_key: str = Form(""),
        model_id: str = Form(""),
        ...
        """One-click add a free provider from the registry as a real
        ModelEndpoint. Optionally pin a default model so it shows up
        immediately in the model picker."""
        require_admin(request)
        from src.free_providers import get_provider
        prov = get_provider(provider_id)
        if not prov:
            raise HTTPException(404, f"Unknown free provider: {provider_id}")
        if prov["needs_api_key"] and not api_key.strip():
            raise HTTPException(400, f"{prov['name']} requires an API key. Get one at {prov.get('key_help', '')}")

        base_url = prov["base_url"]
        # Dedup — don't add a second endpoint with the same base_url.
        db = SessionLocal()
        try:
            existing = db.query(ModelEndpoint).filter(ModelEndpoint.base_url == base_url).first()
            if existing:
                return {"id": existing.id, "name": existing.name, "already_added": True}
            ep_id = str(uuid.uuid4())[:8]
            from src.auth_helpers import get_current_user as _gcu
            _shared_flag = (shared or "").strip().lower() in ("true", "1", "yes")
            _owner_val = None if _shared_flag else (_gcu(request) or None)
            ...
            elif prov.get("models"):
                pinned = [prov["models"][0]]
            ep = ModelEndpoint(
                id=ep_id,
                name=prov["name"],
                base_url=base_url,
                ...
                model_type="llm",
                endpoint_kind="api",
                model_refresh_mode="on_demand",
                pinned_models=json.dumps(pinned) if pinned else None,
                cached_models=json.dumps(prov.get("models", [])) if prov.get("models") else None,
                supports_tools=True,
                owner=_owner_val,
            )
            db.add(ep)
            db.commit()
            # If no default endpoint yet, make this one the default.
            settings = _load_settings()
            enabled_ids = {
                e.id
            ...
            if _default_endpoint_needs_assignment(settings.get("default_endpoint_id") or "", enabled_ids):
                ...
                settings["default_endpoint_id"] = ep.id
                if pinned:
                    settings["default_model"] = pinned[0]
                _save_settings(settings)
            _invalidate_models_cache()
            return {
                "id": ep.id,
