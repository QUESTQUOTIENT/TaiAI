...
def setup_chat_routes(
    ...
    async def chat_endpoint(request: Request, chat_request: ChatRequest) -> Dict[str, str]:
        ...
        # Phase 1.3 - Slim Agent Mode (ChatRequest path).
        slim_profile_setting = chat_request.slim_profile or "auto"
        slim_max_context = chat_request.slim_max_context
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        # Phase 1.3 — Slim Agent Mode settings. Caller passes
        # slim_profile=auto|minimal|balanced|full; slim_max_context as a
        # number to override the profile's default.
        slim_profile_setting = (form_data.get("slim_profile")
                               or (body or {}).get("slim_profile")
                               or "auto")
        slim_max_context_raw = (form_data.get("slim_max_context")
                                or (body or {}).get("slim_max_context"))
        try:
            slim_max_context = int(slim_max_context_raw) if slim_max_context_raw is not None else None
        except (TypeError, ValueError):
            slim_max_context = None
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                try:
                    ...
                    async for chunk in stream_agent_loop(
                        ...
                        slim_profile=(slim_profile_setting or "auto"),
                        slim_max_context=slim_max_context or None,
                    ):
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                data = json.loads(chunk[6:])
