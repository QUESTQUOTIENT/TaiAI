                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in TaiAi (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in TaiAi" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "TaiAi_kind" if "TaiAi_kind" in cols else "'scheduled' AS TaiAi_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                TaiAi_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-TaiAi-Origin"] = "TaiAi-ui"
                outer["X-TaiAi-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", TaiAi_kind or "scheduled")[:64]
                outer["X-TaiAi-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `TaiAi-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `TaiAi_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `TaiAi-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("TaiAi_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `TaiAi_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (TaiAi_INPROCESS_POLLERS=0); "
            "drive `TaiAi-mail poll-scheduled` externally."
        )
        return
    import asyncio
