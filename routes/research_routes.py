...
def setup_research_routes(research_handler, session_manager=None) -> APIRouter:
    ...
    def _emit(bus, type_: str, status: str, **payload) -> None:
        """Emit a PlatformEvent on the shared bus; swallow bus errors.

        Research should keep working even if the events module is
        unavailable. Same defensive pattern as compare_routes.
        """
        if bus is None:
            return
        try:
            from core.events import PlatformEvent
            bus.emit(PlatformEvent(type=type_, status=status, payload=payload))
        except Exception:  # noqa: BLE001 — never let events break research
            pass
    ...
    async def research_cancel(session_id: str, request: Request,
        ...
        _validate_session_id(session_id)
        if not _owns_in_memory(session_id, user):
            _emit(bus, "research.cancel.failed", "failed",
                  session_id=session_id, reason="not_found")
            raise HTTPException(404, "No research found for this session")
        cancelled = research_handler.cancel_research(session_id)
        _emit(bus, "research.cancel.completed", "succeeded",
              session_id=session_id, cancelled=cancelled, owner=user)
    ...
    async def research_result(session_id: str, request: Request,
        ...
        _validate_session_id(session_id)
        if not _owns_in_memory(session_id, user):
            _emit(bus, "research.result.failed", "failed",
                  session_id=session_id, reason="not_found")
            raise HTTPException(404, "No research result available")
        result = research_handler.get_result(session_id)
        if result is None:
            _emit(bus, "research.result.failed", "failed",
                  session_id=session_id, reason="no_result")
            raise HTTPException(404, "No research result available")
        sources = research_handler.get_sources(session_id) or []
        raw_findings = research_handler.get_raw_findings(session_id) or []
        research_handler.clear_result(session_id)
        # This is the natural "research finished" signal — the caller is
        # retrieving the final report. Subscribers (e.g. diagnostics
        # dashboard) use this to mark the research session as delivered.
        _emit(bus, "research.result.delivered", "succeeded",
              session_id=session_id, owner=user,
    ...
    class ResearchStartRequest(BaseModel):
        ...
        # Phase 2.10 - Smart Research presets. tier is "budget" | "balanced"
        # | "deep"; "auto" means let core/research_presets pick from the
        # hardware tier. hardware_tier is optional ("low" | "mid" | "high")
        # and forces a downshift on the budget tier for low-end boxes.
        preset_tier: Optional[str] = Field(
            default="balanced",
            pattern="^(auto|budget|balanced|deep)$",
            description="Smart Research preset tier",
        )
        hardware_tier: Optional[str] = Field(
            default=None,
            pattern="^(low|mid|high)$",
            description="Forced hardware tier override",
        )
    ...
    async def research_start(body: ResearchStartRequest, request: Request):
        ...
        if user == "internal-tool":
            tool_owner = (request.headers.get("X-TaiAi-Owner") or "").strip()
        ...
        # Phase 2.10 - apply Smart Research preset to derive sub-question
        # count (via max_rounds) and pick a model from available endpoints.
        # If body.model is unset, the preset's preferred model keywords
        # drive the choice against the user's configured endpoints.
        try:
            from core.research_presets import build_preset, PRESETS as _RP
        except Exception as _e:  # noqa: BLE001
            _RP = None
            build_preset = None
            logger.debug("[research-preset] import failed: %r", _e)
        if build_preset is not None:
            try:
                from src.model_endpoints import _all_endpoint_labels as _all_labels
                available = _all_labels(user) if _all_labels else []
            except Exception:
                available = []
            tier = (body.preset_tier or "balanced")
            hw = body.hardware_tier
            try:
                preset = build_preset(tier=tier, available_models=available, hardware_tier=hw)
            except Exception as _e:  # noqa: BLE001
                preset = None
                logger.warning("[research-preset] build failed: %r", _e)
            if preset is not None:
                # Cap max_rounds from preset (sub_questions * 2 covers the
                # question + summary rounds). Caller's body.max_rounds (if
                # explicitly set > 0) wins.
                preset_max = max(preset["sub_questions"] * 2, 4)
                if body.max_rounds <= 0:
                    effective_max_rounds = preset_max
                # If caller didn't pin a model, let the preset pick.
                if not body.model and preset.get("chosen_model"):
                    ep_model = preset["chosen_model"]
                    logger.info(
                        "[research-preset] tier=%s picked model=%s (max_rounds=%d)",
                        tier, ep_model, effective_max_rounds,
                    )
