...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    # Trusted upstream-proxy IPs that may set X-Forwarded-For for us. Empty
    # by default — operators running behind nginx/cloudflared/Caddy should
    # add their proxy IPs here so the rate limiter buckets per real client
    # rather than per proxy. Set via env: TaiAi_TRUSTED_PROXIES=1.2.3.4,5.6.7.8
    _TRUSTED_PROXIES = frozenset(
        ip.strip() for ip in (os.getenv("TaiAi_TRUSTED_PROXIES") or "").split(",") if ip.strip()
    )

    def _rate_limit_key(request: Request) -> str:
        """Compute a rate-limit bucket key. Prefers the leftmost X-Forwarded-For
        client IP when the direct connection is from a trusted proxy, otherwise
        falls back to request.client.host. This prevents a single attacker
        from DoS-ing the bucket for the entire user base behind a reverse
        proxy — see THREAT_MODEL.md for the threat this defends against."""
        direct = getattr(request.client, "host", None) or "unknown"
    ...
    async def first_run_setup(body: SetupRequest, request: Request):
        ...
        if not _setup_limiter.check(_rate_limit_key(request)):
    ...
    async def signup(body: SignupRequest, request: Request):
        ...
        if not _signup_limiter.check(_rate_limit_key(request)):
            raise HTTPException(429, "Too many requests — try again later")
    ...
    async def login(body: LoginRequest, request: Request, response: Response):
        if not _login_limiter.check(_rate_limit_key(request)):
            raise HTTPException(429, "Too many requests — try again later")
    ...
    async def toggle_signup(request: Request, response: Response):
        """
        ...
        P2-35: also sets RFC-8594 Deprecation + Sunset headers so callers
        can detect and migrate cleanly.
        """
        user = _get_current_user(request)
        if not user or not auth_manager.is_admin(user):
            raise HTTPException(403, "Admin only")
        auth_manager.signup_enabled = not auth_manager.signup_enabled
        # P2-35: advertise deprecation per RFC 8594 (Deprecation) + RFC 8288 (Sunset).
        # Sunset date is set 6 months out — past then, the endpoint may return 410.
        from datetime import datetime, timedelta, timezone
        sunset = (datetime.now(timezone.utc) + timedelta(days=180)).strftime("%a, %d %b %Y 00:00:00 GMT")
        response.headers["Deprecation"] = "true"
        response.headers["Sunset"] = sunset
        response.headers["Link"] = '</api/auth/open-signup>; rel="successor-version"'
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            # base URL (e.g. `http://host:8091/TaiAi`) — otherwise
            # the topic gets appended after the path and we publish to
            # `/TaiAi/TaiAi` (which ntfy 404s on). ntfy itself
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from TaiAi. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:
