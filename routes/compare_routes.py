...
def _emit(bus: EventBus, type_: str, status: str, **payload) -> None:
    """Emit a PlatformEvent on the shared bus, swallowing bus errors.

    Compare Mode should keep working even if the events module is
    unreachable or a subscriber raises. The bus does its own
    isolation between subscribers; we only protect against a missing
    bus here.
    """
    if bus is None:
        return
    ...
    except Exception:  # noqa: BLE001 — never let events break compare
        pass
...
class CompareMetricsRequest(BaseModel):
    """Phase 2.6 - per-side latency + token usage for a comparison."""
    side: str               # "a" | "b"
    latency_ms: int = 0
    tokens_in: int = 0
    tokens_out: int = 0
...
def _owned_comparison(db, comp_id: str, owner: str):
    """Phase 2.6 helper: owner-scoped lookup of a Comparison row.

    Mirrors the owner-scoping pattern used by _owned_endpoint_by_url: a
    row is visible if the caller's owner matches the row's owner, OR
    if both are None (legacy shared rows + auth-disabled mode). None
    for both authenticated-with-different-owner and not-found rows —
    we never leak existence to a non-owner. Caller is responsible for
    translating None to a 404."""
    if not comp_id:
        return None
    q = db.query(Comparison).filter(Comparison.id == comp_id)
    if owner:
        q = q.filter((Comparison.owner == owner) | (Comparison.owner.is_(None)) | (Comparison.owner == ""))
    else:
        q = q.filter((Comparison.owner.is_(None)) | (Comparison.owner == ""))
    return q.first()
...
def setup_compare_routes(session_manager: SessionManager):
    ...
    def start_comparison(
        ...
        bus: EventBus = Depends(get_default_bus),
    ):
        ...
        Emits ``compare.start.started`` and ``compare.start.completed``
        on the shared event bus. The completion event carries the
        comparison id and the two session ids so other subsystems
        (diagnostics, backup) can correlate downstream activity.
        """
        user = getattr(request.state, 'current_user', None)
        comp_id = str(uuid.uuid4())
        sid_a = str(uuid.uuid4())
        sid_b = str(uuid.uuid4())
        _emit(bus, "compare.start.started", "started",
              comparison_id=comp_id, owner=user,
              model_a=model_a, model_b=model_b, is_blind=is_blind)
        ...
        _emit(bus, "compare.start.completed", "succeeded",
              comparison_id=comp_id, owner=user,
              session_left=session_left, session_right=session_right,
              is_blind=blind)
        return result
    ...
    def vote_comparison(
        ...
        try:
            ...
            if not comp:
                _emit(bus, "compare.vote.failed", "failed",
                      comparison_id=comp_id, reason="not_found")
            ...
            if comp.winner:
                _emit(bus, "compare.vote.failed", "failed",
                      comparison_id=comp_id, reason="already_voted")
            ...
            # The vote is the actual outcome of a comparison — the event
            # analytics cares about. We log the resolved winner + model
            # ids so downstream subscribers (dashboards, leaderboards)
            # can build rankings without re-querying the DB.
            _emit(bus, "compare.vote.recorded", "succeeded",
                  comparison_id=comp_id, owner=user,
                  winner=comp.winner, model_a=comp.model_a,
    ...
    @router.post("/{comp_id}/metrics")
    def post_metrics(request: Request, comp_id: str, body: "CompareMetricsRequest"):
        """Phase 2.6 — record per-side latency + token usage for a comparison.

        Called by the chat_stream SSE handler once each side finishes, so
        the UI can show cost + speed alongside the answers. Owner-scoped:
        only the comparison's owner can write or read metrics."""
        ...
        db = SessionLocal()
        try:
            comp = _owned_comparison(db, comp_id, user)
            if comp is None:
                raise HTTPException(404, "Comparison not found")
            payload = {
                "latency_ms": int(body.latency_ms or 0),
                "tokens_in": int(body.tokens_in or 0),
                "tokens_out": int(body.tokens_out or 0),
                "captured_at": datetime.utcnow().isoformat() + "Z",
            }
            if body.side == "a":
                comp.metrics_a = json.dumps(payload)
            elif body.side == "b":
                comp.metrics_b = json.dumps(payload)
            else:
    ...
    @router.get("/{comp_id}/metrics")
    def get_metrics(request: Request, comp_id: str):
        """Read per-side metrics for a comparison. Returns parsed JSON
        (or null per side if not yet captured) plus the comparison's
        winner + is_blind so the UI can render a compact stats strip."""
        user = get_current_user(request)
        if not user:
            raise HTTPException(401, "auth required")
        db = SessionLocal()
        try:
            ...
            def _parse(s):
                ...
                try:
                    ...
                    return None
            return {
                "metrics_a": _parse(comp.metrics_a),
                "metrics_b": _parse(comp.metrics_b),
                "winner": comp.winner,
                "is_blind": bool(comp.is_blind),
                "model_a": comp.model_a,
                "model_b": comp.model_b,
                "prompt_chars": len(comp.prompt or ""),
                "response_a_chars": len(comp.response_a or ""),
                "response_b_chars": len(comp.response_b or ""),
            }
        finally:
            db.close()
